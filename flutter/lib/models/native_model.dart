...
class PlatformFFI {
  ...
  WegameImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(WegameImpl wegameImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = wegameImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
